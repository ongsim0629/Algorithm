...
### 출력 
 ...
 <h1>배운 내용 정리!</h1>
자바스크립트의 경우 큐를 내장 메소드로 제공하지 않는다

→ shift(), push()로 흉내 낼 수 있지만

shift()는 나머지 배열 원소에 대한 재정렬이 이루어져야 하기 때문에 최악의 경우 O(n)의 시간 복잡도를 가지기 때문에 class로 직접 구현해야한다.

(But, N이 1,000 이하라면 shift를 사용해도 무방하다)
